...
class TextChunker:
    ...
    def get_subchunk(self, chunk_start, chunk_end, sent_embeddings):
        ...
        subchunk_embeddings = [np.zeros(self.dim_emb) for _ in range(dim_qty1)]
        subchunk_qty = 0

        for j in range(chunk_start, chunk_end - 1, 3):
            for k in range(self.dim_emb):
                subchunk_embeddings[subchunk_qty][k] = (
                    sent_embeddings[j][k]
                    + sent_embeddings[j + 1][k]
                    + sent_embeddings[j + 2][k]
                ) / 3
            subchunk_qty += 1

        k = (chunk_end - chunk_start + 1) % 3
        if k == 1:
            for k_ in range(self.dim_emb):
                subchunk_embeddings[subchunk_qty][k_] = sent_embeddings[chunk_end][k_]
        elif k == 2:
            for k_ in range(self.dim_emb):
                subchunk_embeddings[subchunk_qty][k_] = (
                    sent_embeddings[chunk_end][k_] + sent_embeddings[chunk_end - 1][k_]
                ) * 0.5
    ...
    def get_reward1(self, chunks, chunk_qty, sent_embeddings):
        ...
        subchunk_embeddings = [np.zeros(self.dim_emb) for _ in range(chunk_qty * self.dim_qty)]
        dot = [0, 0, 0, 0]
        ...
        for i in range(chunk_qty):
            ...
            subchunk_embeddings[es : es + subchunk_qtys[i]] = self.get_subchunk(
                js, chunks[i], sent_embeddings
            )
            js = chunks[i] + 1
            es += self.dim_qty  # Corrected: Increment by dim_qty, not subchunk_qtys[i]
    ...
    def update_reward(
        ...
        subchunk_embeddings2 = [
            subchunk_embeddings0[es + j].copy() for j in range(2 * self.dim_qty)
        ]
        dot = dot0.copy()

        dot1 = self.get_dot2(
            chunk_qty, subchunk_qtys0, subchunk_embeddings0, ind, ind + 1
        )
        for j in range(4):
            dot[j] -= dot1[j]
        ...
        for i in range(chunks0[ind], js + jk - 1, -1):
            ...
            subchunk_embeddings0[es : es + subchunk_qtys0[ind]] = self.get_subchunk(
                js, i - 1, sent_embeddings
            )
            subchunk_embeddings0[
                es + self.dim_qty : es + self.dim_qty + subchunk_qtys0[ind + 1]
            ] = self.get_subchunk(i, chunks0[ind + 1], sent_embeddings)
            dot2 = self.get_dot2(
                chunk_qty, subchunk_qtys0, subchunk_embeddings0, ind, ind + 1
            )
            for j in range(4):
                dot2[j] += dot[j]  # Corrected: Use the updated 'dot', not 'dot0'
            ...
            if reward2 > reward:
                chunk_ind = i - 1
                chunk_lengths2[0] = (
                    chunk_lengths0[ind] + chunk_lengths0[ind + 1] - chunk_length1
                )
                chunk_lengths2[1] = chunk_length1
                reward = reward2
                subchunk_qtys2 = [subchunk_qtys0[ind], subchunk_qtys0[ind + 1]]
                subchunk_embeddings2[0 : subchunk_qtys0[ind]] = subchunk_embeddings0[
                    es : es + subchunk_qtys0[ind]
                ].copy()
                subchunk_embeddings2[
                    self.dim_qty : self.dim_qty + subchunk_qtys0[ind + 1]
                ] = subchunk_embeddings0[
                    es + self.dim_qty : es + self.dim_qty + subchunk_qtys0[ind + 1]
                ].copy()
                dot = dot2
        ...
        for i in range(chunks0[ind] + 1, chunks0[ind + 1] - jk + 1):
            ...
            subchunk_embeddings0[es : es + subchunk_qtys0[ind]] = self.get_subchunk(
                js, i, sent_embeddings
            )
            subchunk_embeddings0[
                es + self.dim_qty : es + self.dim_qty + subchunk_qtys0[ind + 1]
            ] = self.get_subchunk(i + 1, chunks0[ind + 1], sent_embeddings)
            dot2 = self.get_dot2(
                chunk_qty, subchunk_qtys0, subchunk_embeddings0, ind, ind + 1
            )
            for j in range(4):
                dot2[j] += dot[j]  # Corrected: use updated 'dot'
            m_inter_dot = (
                dot2[self.INTER] / dot2[self.INTER_NO] if dot2[self.INTER_NO] != 0 else 0
            )
            m_intra_dot = (
                dot2[self.INTRA] / dot2[self.INTRA_NO] if dot2[self.INTRA_NO] != 0 else 0
            )
            reward2 = m_intra_dot - m_inter_dot
            if reward2 > reward:
                chunk_ind = i
    ...
    async def chunking(self, text):
        ...
        sent_embeddings = [item.embedding for item in res.data]
        ...
        for i in range(sent3_qty + 1, self.chunk_qty, -1):
            chunk_embeddings = [np.zeros(self.dim_emb) for _ in range(i)]
            for j in range(i):
                for k in range(chunk_starts[j], chunk_ends[j] + 1):
                    chunk_embeddings[j] = np.add(chunk_embeddings[j], sent_embeddings[k])
                divk = 1 / (chunk_ends[j] - chunk_starts[j] + 1)
                chunk_embeddings[j] = chunk_embeddings[j] * divk
        ...
        for _ in range(1):  # Number of iterations
            for ind in range(self.chunk_qty - 1):
                ...
                subchunk_embeddings[
                    es : es + subchunk_qtys[ind]
                ] = result[4][  # Correct slicing
                    0 : subchunk_qtys[ind]
                ]
                subchunk_embeddings[
                    es + self.dim_qty : es + self.dim_qty + subchunk_qtys[ind + 1]
                ] = result[
                    4
                ][  # Correct slicing
                    self.dim_qty : self.dim_qty + subchunk_qtys[ind + 1]
                ]
                dot = result[5]
